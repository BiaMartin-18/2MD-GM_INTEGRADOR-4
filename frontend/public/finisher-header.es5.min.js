...
(function (t) {
  ...
  var h = (function () {
    function h(opts) {
      ...
      // pega container
      var container = this.gr(opts.className);
      ...
      t.addEventListener(
        ...
        function () {
          clearTimeout(timer);
          timer = setTimeout(s.resize.bind(s), 150);
        },
        !1
      );
        ...
        },
        {
          ...
          value: function () {
            ...
            for (var s = 0; s < this.o.ac; s++) {
              var hIndex = s % 4,
                p = new e(this.o.colors.particles[i], hIndex, this.o);
...
// =============================================================
// GARANTE QUE O EFEITO SÓ CARREGA QUANDO O DOM TERMINAR
// =============================================================
window.addEventListener("DOMContentLoaded", () => {
  if (window.finisherHeaderLoaded) return;
  window.finisherHeaderLoaded = true;

  const container = document.querySelector(".finisher-header");
  if (!container) return;

  // CONFIGURAÇÕES DO SEU EFEITO
  new window.FinisherHeader({
    className: "finisher-header",
    count: 20,
    skew: 0,
    shapes: ["c", "s", "t"],
    size: { min: 10, max: 40, pulse: 0.2 },
    speed: { x: { min: 0.1, max: 0.4 }, y: { min: 0.1, max: 0.4 } },
    colors: {
      background: "#0a0f1f",
      particles: ["#31a2f2", "#8ec8ff", "#1e6bb8"],
    },
    opacity: { center: 0.8, edge: 0 },
    blending: "lighten",
  });
});
